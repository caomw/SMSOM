...
__global__ void detection(float* inputH, float* inputS, float* inputV,
	...
	float* thresholdLayer1, float* thresholdLayer2,
	float* ouput,
	float* labelLayerMatch,
	bool* matchLayer1, bool* matchLayer2,
	int width){
		int x = blockDim.x*blockIdx.x + threadIdx.x;
		int y = blockDim.y*blockIdx.y + threadIdx.y;
...
int startFrame = 2, endFrame = 658;

int main(){
	char path[200] = "E:\\数据备份, 以前的研究等\\data收集\\CDnet\\CDnet\\dataset\\dynamicBackground\\fountain02\\input\\in%06d.jpg";
	char fileName[200];
	char outputPath[200] = "E:\\数据备份, 以前的研究等\\data收集\\CDnet\\CDnet\\results\\dynamicBackground\\fountain02\\S2SOM\\bin%06d.png";
	...
	cudaMalloc((void**)&gpuThreshold2, width*height*sizeof(float));
	...
	for(int i = startFrame; i <= endFrame; ++i){
		...
		meanSum<<<grid, block>>>(gpuLayer2[0], gpuLayer2[1], gpuLayer2[2],
			gpuInput[0], gpuInput[1], gpuInput[2],
			gpuMeanDistance2, width);
	}
	...
	calculateThreshold<<<grid, block>>>(gpuMeanDistance2, 
		gpuMaxDistance2, gpuThreshold2, width);
	...
	while (key != 'q'){
		...
		detection<<<grid, block>>>(gpuInput[0], gpuInput[1], gpuInput[2],
			...
			gpuLayer2[0], gpuLayer2[1], gpuLayer2[2],
			gpuMaxDistance1, gpuMaxDistance2,
			gpuOutput,
			gpuLabelLayerMatch,
			gpuMatch1, gpuMatch2,
			width);
